 ...
 * <br>
 */
public class ValidatorMultiColumna extends AbstractValidator {
	...
	public boolean isValid(HttpServletRequest req, Recordset inputParams,
		...
		boolean b3 = attribs.containsKey("colname2");
		if ((!b2)&&(!b3))
		...
		if(rs != null) {
			...
			RecordsetField rf2 = inputParams.getField((String)attribs.get("colname2"));
			switch (rf.getType()) {
	
				case java.sql.Types.DATE:
				case java.sql.Types.TIMESTAMP:
					//verifica que ya exista el valor en el recordset
					if ((rs.findRecord((String)attribs.get("colname"), inputParams.getDate((String)attribs.get("colname"))) != -1)&&(rs.findRecord((String)attribs.get("colname2"), inputParams.getDate((String)attribs.get("colname2"))) != -1)) {
				...
				case java.sql.Types.INTEGER:	
					//verifica que ya exista el valor en el recordset
					if ((rs.findRecord((String)attribs.get("colname"), inputParams.getInteger((String)attribs.get("colname"))) != -1)&&(rs.findRecord((String)attribs.get("colname2"), inputParams.getInteger((String)attribs.get("colname2"))) != -1)){
						//si el recordset del request contiene un parametro con el nombre rowindex ignora el recordNumber
						//que corresponde al mismo registro
						if (inputParams.containsField("rowindex")) {
							if(rs.getRecordNumber() != inputParams.getInt("rowindex"))
				...
				default:
					//verifica que ya exista el valor en el recordset
					if ((rs.findRecord((String)attribs.get("colname"), inputParams.getString((String)attribs.get("colname"))) != -1)&&(rs.findRecord((String)attribs.get("colname2"), inputParams.getString((String)attribs.get("colname2"))) != -1) ) {
						//si el recordset del request contiene un parametro con el nombre rowindex ignora el recordNumber
						//que corresponde al mismo registro
						if (inputParams.containsField("rowindex")) {
							if(rs.getRecordNumber() != inputParams.getInt("rowindex"))
