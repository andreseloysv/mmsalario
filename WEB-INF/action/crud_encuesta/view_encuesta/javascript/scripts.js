...
function editar(fila){
	...
	var campos = "<form name='edita_encuesta' id='edita_encuesta' enctype='multipart/form-data' method='POST' action='http://localhost/mmsalario/action/crud_encuesta/view_encuesta/upload' target='uploadFrame'  style='display: none;' >"; 
	var contador=1;
	var nombre="";
	var descripcion="";
	...
	campos+='<input type="hidden" id="nombre_encuesta" name="nombre_encuesta" value="'+nombre+'" >';
	campos+='<input type="hidden" id="descripcion_encuesta" name="descripcion_encuesta" value="'+descripcion+'" >';
...
function guardaFoto(){
	 ...
	 document.getElementById("edita_encuesta").submit();
...
function upload()
{
	...
	if (document.edita_encuesta.id.value=="") {
		
		if (document.edita_encuesta.file.value=="") {
			alert("Por favor indique el archivo a ser cargado.");
			document.edita_encuesta.file.focus();
			return false;
		}
		if (document.edita_encuesta._tempfile.value=="") {
			document.edita_encuesta.submit.disabled = true;
			document.getElementById("status").style.display="";
			return true;
		} else {
			save();
...
function setFileInfo(tempFile, fileName, fileSize, contentType) {
	document.edita_encuesta._tempfile.value = tempFile;
	document.edita_encuesta._filename.value = fileName;
	document.edita_encuesta._filesize.value = fileSize;
	document.edita_encuesta._contenttype.value = contentType;
}
...
//restaurar el formulario en caso de error
function retryAddnewOrEdit() {
	document.edita_encuesta.submit.disabled=false;
	setFocusOnForm("edita_encuesta");		
}
