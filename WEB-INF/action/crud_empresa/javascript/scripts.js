...
function insert()
{
		...
		var str=$("#profile-image").find("img")[0].src;
...
//Valida que los campos de un formulario este llenos

function validaNombreMuestra(){
	
	var esValido=true;
	if($("#form2_error").length>0){
		$("#form2_error").remove();
	}
	...
	$(".input_mater_detail").each( function( key, value ) {
		if(value.value==""){
			$('<div>').attr({
			    id: 'nombre_muestra_form_error_1',
			    'class': 'errormsg'
			}).appendTo($(value).parent());
			$("#nombre_muestra_form_error_1").text("Este valor DEBE ser ingresado.");
			esValido= false;
		}
	});	
