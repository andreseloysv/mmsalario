...
/**
 * Crear objeto XmlHttpRequest de manera portable entre browsers IE y FF.
 * Retorna una referencia al objeto listo para usarse en comunicacion Ajax
 */
...
function cargaTabla(text){
	$('#tabla_excel tr:last').after($.parseHTML(text));
	console.log("cargaTabla");
}
/**
 * Invocar servicio Ajax httpMethod: String GET o POST uri: Path del servicio,
 * no incluir el contexto. Ejemplo: /action/xxxx/yyyy divResponse: ID del DIV
 * donde se muestra la respuesta en caso de que sea HTML divProgress: ID del DIV
 * a mostrar mientras se ejecuta la llamada formName: Nombre del formulario en
 * caso que httpMethod sea igual a POST afterResponseFn: Apuntador a funcion a
 * ejecutar despues de mostrar una respuesta HTML onErrorFn: Apuntador a funcion
 * a ejecutar en caso de error http 500, 403, 404, 401, etc. Puede utilizarse
 * para restaurar los DIVs que se escondieron antes de invocar esta funcion.
 */
		...
		http.onreadystatechange = function ()
		{
			if (http.readyState == S_READY) 
			{
				...
				else 
				{
					...
					{
						// tratar caso cuando no hay comunicacion con el
						// servidor
						if (contentType==null || (http.status !=200 && http.status!=400)) {
							return ("ContentType: "+contentType+" http.status: "+http.status+" ");
...
/**
 * Limpiar los controles de un formulario, los comboBox se ubican en el 1er
 * elemento, el atributo value del resto de los controles se pone en "" vacio.
 * El focus se pone en el 1er control activo. formName: nombre del formulario
 */
...
/**
 * Poner el focus en el 1er control activo de un formulario. formName: Nombre
 * del formulario
 */
...
/**
 * Posicionar un DIV justo debajo de un control de formulario, alineado a la
 * izquierda. element: objeto que representa al control box: objeto que
 * representa al DIV
 */
function moveBox(element, box) 
{
	...
	/*
	 * NOTE: Original author: JTricks.com :: http://www.jtricks.com/
	 */
 ...
 * abajo y alineado a la izquierda del elemento cuyo ID es el valor del
 * parámetro objID objID: ID del DIV que sera creado o inyectado el IFRAME
 * width: Ancho en pixeles del DIV height: Altura en pixeles del DIV centerPage:
 * Variable que determina si el DIV sera colocado debajo de un elemento del HTML
 * o centrado en la pagina HTML, si es nulo quiere decir que sera colocado
 * debajo del un elemento, si es true sera centrado en la pagina
 */		
...
/**
 * Esconde un DIV + IFRAME abierto con la función showBox() objID: ID del DIV
 * que sera ocultado
 */
...
/**
 * Retorna TRUE si el valor representa una fecha de lo contrario retorna FALSE.
 * Si el año viene con 2 dígitos y en menor que 30, se asume el 20XX, de lo
 * contrario 19XX. value: String que representa una fecha con formato dd-mm-yyyy
 * o dd/mm/yyyy
 */
...
/**
 * Retorna un objeto DATE si el valor representa una fecha de lo contrario
 * retorna NULL. Si el año viene con 2 dígitos y es menor que 30, se asume el
 * 20XX, de lo contrario 19XX. value: String que representa una fecha con
...
/**
 * Mostrar un Calendario debajo y alineado a la izquierda del elemento designado
 * por el parametro objID. El elemento debe ser un textbox. objID: ID del
 * elemento en donde sera colocada la fecha seleccionada lboundID: ID del
 * elemento que contiene una fecha menor que sera usada para no permitir
 * seleccionar fechas menores en el calendario a la fecha en el elemento objID,
 * permite nulos cuando no se requiere. uboundID: ID del elemento que contiene
 * una fecha mayor que sera usada para no permitir seleccionar fechas mayores en
 * el calendario a la fecha en el elemento objID, permite nulos cuando no se
 * requiere.
 */
...
/**
 * Retorna valor seleccionado en el calendario poniendolo dentro del textbox
 * asociado y cierra el calendario objID: ID del textbox asociado al calendario
...
function calendarReturnValue(objID, sdate)
{
 ...
 console.debug( dia+"-"+mes+"-"+n[2]);
	
	document.getElementById(objID).value = dia+"-"+mes+"-"+n[2];
...
/**
 * Filtrar caracteres no permitidos en campos fecha, se usa de esta manera:
 * document.forms1.text1.onkeypress = keypressDate en el evento onload del body.
 */
...
/**
 * Filtrar caracteres no permitidos en campos enteros, se usa de esta manera:
 * document.forms1.text1.onkeypress = keypressInteger en el evento onload del
 * body.
 */
...
/**
 * Filtrar caracteres no permitidos en campos con decimales, se usa de esta
 * manera: document.forms1.text1.onkeypress = keypressDouble en el evento onload
 * del body. Solo acepta el punto "." como separador de decimales.
 */
...
/**
 * Retorna la trama requerida para hacer un POST del formulario indicado usando
 * Ajax formName: Nombre del formulario
 */
...
/**
 * Retorna checkboxes o radiobutton con la propiedad checked = true radioName:
 * Nombre del checkbox o radiobutton radioValue: Valor del checkbox o
 * radiobutton formName: Nombre del formulario
 */
...
/**
 * Retorna combos con la propiedad select = true comboName: Nombre del combo
 * comboValue: Valor del combo formName: Nombre del formulario
 */
...
/**
 * Remplazar el valor "," por "." en un elemento que contiene un valor de tipo
 * decimal. elem: Elemento que contiene el valor a ser remplazado
 */
...
/**
 * Funciones de soporte para los grid contiene funciones para la vista pagina,
 * panel de control, regresar a la pagina indicada, y ordenamiento por columnas.
 */
...
/**
 * Ir a la pagina indicada por la variable currentPage url: URI donde se
 * mostrara la vista pagina
 */
...
/**
 * Ordenar el grid por la columna indicada recordsetId: ID del recordset
 * almacenado en sesion en el servidor colName: columna del recordset por la
 * cual se hara el sort
 */
...
/**
 * Abre un picklist DIV + IFRAME id: ID del elemento donde sera posicionado el
 * picklist, y sera llenado el valor de tipo PK (comunmente) seleccionado en el
 * picklist idCtl: ID del elemento en donde sera llenado el texto seleccionado
 * en el picklist url: URL en donde se encuentra el action que sera mostrado
 * como picklist ancho: Ancho del picklist altura: Altura del piklist
 */
 ...
 * 
 * @param id
 *            Valor comunmente de tipo PK seleccionado en el picklist
 * @param description
 *            Valor comunmente de tipo texto seleccionado en el picklist
 * @return
 */
...
/**
 * Obtener el nodo a partir de el nombre del control node: Nodo name: Nombre del
 * control
 */
...
/**
 * Añadir un item a un listbox elementId: ID del control que representa al
 * ListBox itemId: ID del item itemTitle: Texto del item Notas: no permite
 * añadir items con ID duplicado
 */ 
...
/**
 * Remover un item a un listbox elementId: ID del control que representa al
 * ListBox Nota: solo removera el item si esta seleccionado
 */ 
...
/**
 * Eliminar todos los items de un listbox elementId: ID del control que
 * representa al ListBox
 */
...
/**
 * Obtener los IDs de los items de un listbox concatenados como un String y
 * separados por un caracter, por defecto es ";" si no se especifica elementId:
 * ID del control que representa al ListBox separator: caracter a utilizar para
 * separar los items Nota: si el control esta vacio retorna un string vacio
 */ 
...
/**
 * Llenar el contenido de un objeto, como un DIV por ejemplo. Forma parte del
 * soporte para la validacion de formularios posteados via Ajax, es utilizada
 * elementId: Elemento donde sera llenado el contenido de un objeto text: Texto
 * o valor a ser llenado
 */
...
/**
 * Verifica si existe el DIV que corresponde a un control de un formulario para
 * mostrar un mensaje de error, si no existe lo crea y luego muestra el mensaje
 * usando setInnerHtml. Es utilizada por el servicio generico de validacion
 * Ajax: /action/error/validation/ajax formElementId: Elemento donde sera
 * mostrado el mensaje de error text: Texto de error
 */
...
/**
 * By Dustin Diaz - http://www.dustindiaz.com/top-ten-javascript/ Retorna un
 * array de elementos que tengan una clase dada, los parametros node y tag
 * pueden ser null searchClass: Clase node: Nodo tag: TagName
 */
...
/**
 * Código tomado de:
 * http://www.codingforums.com/showthread.php?p=178077#post178077 Permite dar
 * formato a un decimal y mostrar un cierto numero de decimales n: Numero de
 * decimales a mostrar
 */
...
/**
 * Código tomado de: http://www.webdeveloper.com/forum/showthread.php?t=68675
...
/**
 * Muestra un DIV + IFRAME en el centro de una pagina dialogID: ID del dialog
 * que sera centrado url: URL del action que contiene la informacion que sera
 * impresa dentro del DIV + IFRAME ancho: Ancho del DIV + IFRAME altura: Altura
 * del DIV + IFRAME
 */
...
/**
 * Oculta un DIV + IFRAME que se encuentra en el centro de una pagina
 */
...
/*
 * ! jQuery UI - v1.9.2 - 2013-06-04 http://jqueryui.com Includes:
...
$.widget("ui.droppable", {
	...
	_over: function(event) {
		...
		var draggable = $.ui.ddmanager.current;
		if (!draggable || (draggable.currentItem || draggable.element)[0] == this.element[0]) return; // Bail
																										// if
																										// draggable
																										// and
																										// droppable
	...
	_drop: function(event,custom) {
		...
		if (!draggable || (draggable.currentItem || draggable.element)[0] == this.element[0]) return false; // Bail
																											// if
																											// draggable
																											// and
																											// droppable
...
$.ui.ddmanager = {
	...
	prepareOffsets: function(t, event) {
		...
		droppablesLoop: for (var i = 0; i < m.length; i++) {
			...
			if(m[i].options.disabled || (t && !m[i].accept.call(m[i].element[0],(t.currentItem || t.element)))) continue;	// No
																															// disabled
																															// and
																															// non-accepted
			for (var j=0; j < list.length; j++) { if(list[j] == m[i].element[0]) { m[i].proportions.height = 0; continue droppablesLoop; } }; // Filter
																																				// out
																																				// elements
																																				// in
																																				// the
																																				...
																																				// dragged
																																				// item
			m[i].visible = m[i].element.css("display") != "none"; if(!m[i].visible) continue; 									// If
																																// the
																																// element
																																// is
																																// not
			...
			if(type == "mousedown") m[i]._activate.call(m[i], event); // Activate
																		// the
																		// droppable
																		// if
																		// used
...
$.widget("ui.sortable", $.ui.mouse, {
		...
		for (var i = this.items.length - 1; i >= 0; i--) {
			...
			if (itemElement != this.currentItem[0] // cannot intersect with
				...
				&&	this.placeholder[intersection == 1 ? "next" : "prev"]()[0] != itemElement // no
																								...
																								// done
																								// before
				&&	!$.contains(this.placeholder[0], itemElement) // no action
																	// if the
																	// item
																	// moved is
																	// the
	...
	_rearrange: function(event, i, a, hardRefresh) {
		...
		this._delay(function() {
			if(counter == this.counter) this.refreshPositions(!hardRefresh); // Precompute
																				// after
																				// each
																				// DOM
																				// insertion,
	...
	_clear: function(event, noPropagation) {
		...
		if((this.fromOutside || this.domPosition.prev != this.currentItem.prev().not(".ui-sortable-helper")[0] || this.domPosition.parent != this.currentItem.parent()[0]) && !noPropagation) delayedTriggers.push(function(event) { this._trigger("update", event, this._uiHash()); }); // Trigger
																																																																							// update
																																																																							// callback
																																																																							// if
																																																																							// the
		...
		if(this._storedCursor) $('body').css("cursor", this._storedCursor); // Reset
																			// cursor
		if(this._storedOpacity) this.helper.css("opacity", this._storedOpacity); // Reset
																					// opacity
		if(this._storedZIndex) this.helper.css("zIndex", this._storedZIndex == 'auto' ? '' : this._storedZIndex); // Reset
																													// z-index
		...
		if(this.cancelHelperRemoval) {
			if(!noPropagation) {
				...
				for (var i=0; i < delayedTriggers.length; i++) { delayedTriggers[i].call(this, event); }; // Trigger
																											// all
																											// delayed
																											// events
...
$.widget( "ui.accordion", {
	...
	refresh: function() {
		...
		if ( heightStyle === "fill" ) {
			// IE 6 treats height like minHeight, so we need to turn off
			// overflow
			// in order to get a reliable height
			// we use the minHeight support test because we assume that only
			// browsers that don't support minHeight will treat height as
			// minHeight
...
$.widget("ui.dialog", {
	...
	_size: function() {
		 ...
		 * .ui-dialog-content divs will both have width and height set, so we
		 * need to reset them
		 */
...
$.extend( $.ui.dialog.overlay, {
	...
	resize: function() {
		/*
		 * If the dialog is draggable and the user drags it past the right edge
		 * of the window, the document becomes wider so we need to stretch the
		 * overlay. If the user then drags the dialog back to the left, the
		 * document will become narrower, so we need to shrink the overlay to
		 * the appropriate size. This is handled by shrinking the overlay before
    ...
    $.extend({
        ...
        function () {
            ...
            function buildParserCache(table, $headers) {
                ...
                if (table.tBodies.length == 0) return; // In the case of empty
														// tables
  ...
  if(field.value != '') {
    if(regs = field.value.match(re)) {
      if(regs[1] < 1 || regs[1] > 31) {
        errorMsg = "Dia invalido: " + regs[1];
      } else if(regs[2] < 1 || regs[2] > 12) {
        errorMsg = "Mes invalido: " + regs[2];
      } else if(regs[3] < minYear || regs[3] > maxYear) {
        errorMsg = "Año invalido: " + regs[3] + " - debe estar entre " + minYear + " y " + maxYear;
      }
    } else {
      errorMsg = "Formato de fecha invalido, deberia ser asi: dd-mm-aaaa ";
    }
  } else if(!allowBlank) {
    errorMsg = "No a colocado una fecha.";
  }
...
// $(function() {
    ...
    function runEffect() {
      ...
      // run the effect
      $( "#imagen_archivo_excel" ).hide( selectedEffect, options, 1000, callback );
    };
 
    // callback function to bring a hidden box back
    function callback() {
      setTimeout(function() {
        $( "#imagen_archivo_excel" ).removeAttr( "style" ).hide().fadeIn();
      }, 1000 );
    };
}
